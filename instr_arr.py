...
class _I(Instruction):
	...
	def compute_instr(self, instr, rs1, imm, rd):
		#print(f"compute_instr called with: instr={instr}, rs1={rs1}, imm={imm}, rd={rd}")
		instr = super().check_instr_valid(instr, I_instr)
		opcode, f3 = 0, 1
		#print("imm_value_I: " + str(imm))
		return "".join([
			_I.immediate(imm),
			super().reg(rs1),
			instr_map[instr][f3],
	...
	def immediate(imm):
		if imm is not None:
			...
			# Handle the case when imm is None
			imm_value = 0  # or any other default value that makes sense in your context
		return format(((1 << 12) - 1) & imm_value, '012b')
...
class _S(Instruction):
	...
	def compute_instr(self, instr, rs1, rs2, imm):
		#print(f"compute_instr called with: instr={instr}, rs1={rs1}, rs2={rs2}, imm={imm}")
		instr = super().check_instr_valid(instr, S_instr)
		opcode, f3 = 0, 1
		#print("imm_value_S: " + str(imm))
		return "".join([
			_S.immediate(imm, 1),
			super().reg(rs2),
			super().reg(rs1),
	...
	def immediate(imm, n):
		...
		'''
		1<<12 = 4096 - 1 = 4095 => 4095 = 111111111111 (create bitmask for 12 bits)
		'''
		mod_imm = format(((1 << 12) - 1) & int(imm), '012b')
		#print("mod_imm: " + str(mod_imm))
		#split imm[11:0] and insert into 32-bit instruction format
		if n == 1:
			#print("mod_imm[0] + mod_imm[12-11 : 12-5] = " + mod_imm[0] + mod_imm[12-11 : 12-5])
			return mod_imm[0 : 12 - 5]
		#print("mod_imm[12-5 : 12 - 0] = " + mod_imm[12-5 : 12 - 0])
		return mod_imm[12-5 : 12 - 0]
...
class _I_parse(InstructionParser):
	...
	def organize(self, tokens):
		...
		if instr == "jalr":
			...
			rs1, imm, rd = reg_map[tokens[3]], tokens[2], reg_map[tokens[1]]
		elif instr == 'addi' or instr == 'andi' or instr == 'slti':
